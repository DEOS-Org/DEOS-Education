...
export const getAcademicPerformanceReport = async (req: Request, res: Response) => {
  try {
    const {
      fecha_desde,
      fecha_hasta,
    ...
    const reporte = await reportService.generateAcademicPerformanceReport(filtros);
    ...
    console.error('Error in getAcademicPerformanceReport:', error);
    res.status(error.statusCode || 500).json({
      message: error.message || 'Error interno del servidor'
    });
  }
...
export const getStatisticsReport = async (req: Request, res: Response) => {
  try {
    ...
    const reporte = await reportService.generateStatisticsReport(filtros);

    res.json(reporte);
  } catch (error: any) {
    console.error('Error in getStatisticsReport:', error);
    res.status(error.statusCode || 500).json({
      message: error.message || 'Error interno del servidor'
    });
  }
...
export const exportAttendanceToExcel = async (req: Request, res: Response) => {
  try {
    ...
    const reporte = await reportService.generateAttendanceReport(filtros);
    const excelBuffer = await reportService.exportToExcel(reporte, 'asistencia');

    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename=reporte-asistencia-${Date.now()}.xlsx`);
    res.send(excelBuffer);
  } catch (error: any) {
    console.error('Error in exportAttendanceToExcel:', error);
    res.status(error.statusCode || 500).json({
      message: error.message || 'Error interno del servidor'
    });
  }
...
export const exportAttendanceToPDF = async (req: Request, res: Response) => {
  try {
    ...
    const reporte = await reportService.generateAttendanceReport(filtros);
    const pdfBuffer = await reportService.exportToPDF(reporte, 'asistencia');

    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename=reporte-asistencia-${Date.now()}.pdf`);
    res.send(pdfBuffer);
  } catch (error: any) {
    console.error('Error in exportAttendanceToPDF:', error);
    res.status(error.statusCode || 500).json({
      message: error.message || 'Error interno del servidor'
    });
  }
...
export const exportPerformanceToExcel = async (req: Request, res: Response) => {
  try {
    const {
      fecha_desde,
      fecha_hasta,
    ...
    const reporte = await reportService.generateAcademicPerformanceReport(filtros);
    const excelBuffer = await reportService.exportToExcel(reporte, 'rendimiento');

    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename=reporte-rendimiento-${Date.now()}.xlsx`);
    res.send(excelBuffer);
  } catch (error: any) {
    console.error('Error in exportPerformanceToExcel:', error);
    res.status(error.statusCode || 500).json({
      message: error.message || 'Error interno del servidor'
    });
  }
...
export const exportPerformanceToPDF = async (req: Request, res: Response) => {
  try {
    const {
      fecha_desde,
      fecha_hasta,
    ...
    const reporte = await reportService.generateAcademicPerformanceReport(filtros);
    const pdfBuffer = await reportService.exportToPDF(reporte, 'rendimiento');

    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename=reporte-rendimiento-${Date.now()}.pdf`);
    res.send(pdfBuffer);
  } catch (error: any) {
    console.error('Error in exportPerformanceToPDF:', error);
    res.status(error.statusCode || 500).json({
      message: error.message || 'Error interno del servidor'
    });
  }
